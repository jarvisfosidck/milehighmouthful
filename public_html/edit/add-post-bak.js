	...
	*/
	...
	var addPostFilesUploaded = [];
	document.getElementById("handleUpLoadFiles").addEventListener("change", handleFiles, false);//hidden element for upload with click and not drag drop

function handleFiles() { 

	window.addPostFilesUploaded.push(this.files);
	processFileUpload(this.files);

}

function dropImageUpload(event) {
...
function processFileUpload(files) {
	 ...
	 var el = document.getElementById("imageDropBucketArea");
	 
  for (var i = 0; i < files.length; i++) {
	window.addPostFilesUploaded.push(files[i]);
   ...
   var img = document.createElement("img");
	...
	el.className = el.className.replace(/\?ondragEnter/,'');
}
...
function assignClicksLocal() {
	...
	var addPostClickItems = ["deleteThisPost"
...
//		console.log(this,evt,target.id,this.id);
		...
		switch (id) {
			...
			case "imageDropBucketArea" : 
			 imageDropBucketArea(el,evt); 
			 break;
...
var jsonPostModel = {
	...
	for (var i=0,len=addPostFilesUploaded.length;i<len;i++) {
		var fname = "image"+i;
		//console.log(addPostFilesUploaded[i],addPostFilesUploaded[i]['name']);
		formData.append(fname, addPostFilesUploaded[i]);	
	}
...
function imageDropBucketArea (el,evt) { //may not be needed

//console.log(evt, 'wow, works', el);
}

function addAnotherImage (el,evt) {
	
	document.getElementById("handleUpLoadFiles").click();
